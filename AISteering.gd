...
# Avoids walls
func apply_collision_avoidance(body : KinematicBody2D, distance : float, repel := false):
	var separation = 2 * PI / context.size()
	var v = Vector2()
	for i in context.size():
		var angle = i * separation
		var vel = polar2cartesian(distance, angle)
...
func apply_separation(position : Vector2, neighbors : Array, radius : float, weight := 1.0):
	...
	for neighbor in neighbors:
		...
		if is_zero_approx(distance):
			away += polar2cartesian(1, rand_range(-PI, PI))
		# Else move away from
